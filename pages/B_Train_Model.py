...
if df is not None:
    ...
    tfidf_word_count_data = None

    tfidf_word_count_data = df.loc[:, df.columns.str.startswith('tf_idf_word_count_')]
    if(len(tfidf_word_count_data.columns)):
        #st.write('len(tfidf_word_count_data.columns): {}'.format(len(tfidf_word_count_data.columns)))
        #st.dataframe(tfidf_word_count_data)
        word_count_encoder_options.append('TF-IDF')

    word_count_data = df.loc[:, df.columns.str.startswith('word_count_')]
    if(len(word_count_data.columns)):
        #st.write('len(word_count_data.columns): {}'.format(len(word_count_data.columns)))
